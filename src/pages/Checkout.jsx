...
const Checkout = ({ onNavigateToCart, onNavigateToSuccess, onNavigate, activeTab, cartCount, query, setQuery, onAdminClick }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    clearCart();
    // Redirect to UPI payment if on mobile
    const isMobile = /Android|iPhone|iPad|iPod|Opera Mini|IEMobile|WPDesktop/i.test(navigator.userAgent);
    const upiAmount = encodeURIComponent(getTotalPrice());
    const upiUrlValue = `upi://pay?pa=7569460743@superyes&pn=Taaza%20Meat&am=${upiAmount}&cu=INR`;
    setUpiUrl(upiUrlValue);
    if (isMobile) {
      window.location.href = upiUrlValue;
      setTimeout(() => setShowUpiLink(true), 2000);
    }
    onNavigateToSuccess(docRef.id);
  };
    ...
    <div className="min-h-screen bg-gray-50 pb-20 md:pb-0">
      ...
      {showUpiLink && upiUrl && (
        <div className="mt-6 text-center">
          <a href={upiUrl} style={{display: 'inline-block', color: 'blue', fontWeight: 'bold', fontSize: 18}} target="_blank" rel="noopener noreferrer">
            Pay with UPI (PhonePe, GPay, Paytm)
          </a>
          <div className="text-gray-500 text-sm mt-2">If you are not redirected automatically, tap the link above to pay.</div>
        </div>
      )}
    </div>
  );
