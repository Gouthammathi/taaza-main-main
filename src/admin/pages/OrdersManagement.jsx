                ...
                <tr key={order.docId || order.id} className="hover:bg-gray-50 cursor-pointer" onClick={() => setSelectedOrder(order)}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-center">
                    {order.id && order.id.startsWith('CUS-') && (
                      order.status === 'paid' ? (
                        <span title="Paid" className="inline-block text-green-600 text-3xl">
                          <MdCheckCircle />
                        </span>
                      ) : (
                        <button onClick={async e => {
                          ...
                          await updateDoc(doc(db, 'orders', order.docId), { status: 'paid' });
                        }} title="Mark as Paid" className="text-green-600 hover:text-green-800 text-2xl">
                          <MdCheckCircle />
                        </button>
                      )
                    )}
      ...
      {selectedOrder && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl p-8 border border-gray-200">
            ...
            <div>
              <div className="text-sm text-gray-500 mb-2 font-semibold">Products</div>
                ...
                {selectedOrder.items.map((item, idx) => (
                  <div key={idx} className="flex items-center justify-between px-4 py-3 bg-white hover:bg-gray-50 transition">
                    ...
                    <div className="flex items-center gap-6">
                      ...
                      <div className="text-sm text-gray-700">Total: <span className="font-semibold">₹{item.total}</span></div>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div id="receipt-print-area" ref={printRef} style={{ width: '100vw', zIndex: 9999, background: 'white', padding: 0, margin: 0 }}>
          ...
          <style>{`
            @media print {
              ...
              @page { margin: 0; }
              #receipt-print-area > div { width: 100vw !important; max-width: 100vw !important; border-radius: 0 !important; box-shadow: none !important; padding: 0 !important; }
            }
          `}</style>
        </div>
      )}
